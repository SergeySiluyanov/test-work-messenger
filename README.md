...
При первом запуске приложения необходимо уставить зависимости.

```bash
npm install
```

Версии пакетов зависимостей фиксированы, поэтому при последующей переустановке пакетов достаточно будет выполнить команду

```bash
npm ci
```

Для локального запуска необходимо запустить frontend и API server.
